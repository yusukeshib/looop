...
- WORKSPACE: you start in the loop data dir (read-only context for you, save the
  ...
  sandbox FIRST and cd into it — never edit code in the data dir:
    • if `box` is available:  box workspace add __SESSION__ --repo <source> && cd "$(box workspace switch __SESSION__)"
                              (box v0.3+; aliases box ws add / box ws sw. <source> is a registered source name — `box source list`.)
